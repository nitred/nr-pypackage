...
def render_template(template_filename, **template_kwargs):
    ...
    if is_file_render_exempt(template_filename):
        ...
        return Template(
            file_contents, trim_blocks=True, lstrip_blocks=True, keep_trailing_newline=True
...
def get_rendered_file(file_path_template, template_kwargs):
    ...
    assert os.path.isfile(
        file_path_template
    ), "Template file {file_path_template} must be a FILE but is not.".format(
...
def get_generator_of_template_filenames_and_renders(
    CWD, PACKAGE_DIR, package_type, template_kwargs
):
    ...
    TEMPLATES_DIR = pkg_resources.resource_filename(
        "nr_pypackage", "templates/{}".format(package_type)
    )

    for root, dirs, files in os.walk(TEMPLATES_DIR):
        for filename in files:
            print(f"Rendering filename: {filename}")
            ...
            if is_ignore_file(
                file_path_template, blueprints_dict=template_kwargs.get("blueprints", {})
            ):
                continue
            ...
            if "BP_DATABASE" in file_path_template and "TABLE_NAME" in file_path_template:
                ...
                for table_name, table_details in bp_tables.items():
                    ...
                    template_kwargs["blueprints"]["database"]["current_table"] = bp_tables[
                        table_name
                    ]
                    ...
                    file_path_package = get_file_path_package(
...
def create_package(
    ...
    template_kwargs = get_template_kwargs(
        package_name, author_name, author_email, scm_url, scm_username, blueprints
    )
    ...
    try:
        ...
        for package_filename, rendered_file in get_generator_of_template_filenames_and_renders(
            CWD, PACKAGE_DIR, package_type, template_kwargs
        ):
            # Create file.
            if dry_run is False:
                ...
                print(
                    "(DRY-RUN) Creating file: {package_filename}".format(
